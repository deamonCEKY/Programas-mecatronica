...
class Login(Control):
   ...
   def init (self, initPassword):
      self.storedPassword = initPassword
      
   def reloadHandler(self):
      VFP.create(self.id, "Login")
   
   def setTitle(self, newTitle):
      VFP.call(self.id, "setTitle", '"'+newTitle+'"')
   ...
   def __call__(self):
      return VFP.isAuthorized()

   def eventHandler(self, loginRequest):

      if loginRequest != "0":
         loginRequest = loginRequest.replace('"', '')
         loginRequest = loginRequest.split(',')
         enteredPassword = loginRequest[1]

         if enteredPassword == self.storedPassword:
            VFP.authorize(True)
         else:
            VFP.call(self.id, "authError")
      else:
         self.logOut()
